...
echo "Проверка наличия директории для хранения бекапов..."s
...
pg_dump --dbname=$CONNECT_DB | pv -N "Загружено" > $STORAGE/$(date +%Y-%m-%d).sql | gzip > $STORAGE/$(date +%Y-%m-%d).sql.gz
...
. $NOTICE/email.sh "$SERVER_NAME: backup $(date +%Y-%m-%d) готов!" "$SPACE_USED"
. $NOTICE/telegram.sh "$SERVER_NAME: backup $(date +%Y-%m-%d) готов!" "$SPACE_USED"
