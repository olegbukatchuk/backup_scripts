#!/bin/bash

# DB Suite роутер
# Автор:  Олег Букатчук
# Версия: 1.9
# e-mail: oleg@bukatchuk.com

# Подключаем настройки
source ./$DB_SUITE/db_suite.sh

# Тип БД (1-й параметр к скрипту)
export TYPE=$1

# Действие с БД (2-й параметр к скрипту)
export ACTION=$2

# Зеленый цвет
export SETCOLOR_SUCCESS="echo -en \\033[1;32m"

# Красный цвет
export SETCOLOR_FAILURE="echo -en \\033[1;31m"

# Цвет по умолчанию
export SETCOLOR_NORMAL="echo -en \\033[0;39m"

echo -e "Задание выполняется..."

# Команда, которую нужно отследить
sudo

#####################################################################################################################
# Проверка аргументов перед запуском скриптов

# 1. $#	переменная, содержащая количество аргументов, преданных скрипту.
# 2. -eq 2	проверяем, равняется ли количество переданных аргументов двум.
# 3. $0	возвращает путь к shell скрипту.
[ $# -eq 2 ] && { echo "Usage: $0 argument1 argument2"; exit 1; }

#####################################################################################################################
# Цветной вывод результата выполнения скриптов

if [ $? -eq 0 ];
    then
        ${SETCOLOR_SUCCESS}
        echo -n "$(tput hpa $(tput cols))$(tput cub 6)[OK]"
        ${SETCOLOR_NORMAL}
        echo
    else
        ${SETCOLOR_FAILURE}
        echo -n "$(tput hpa $(tput cols))$(tput cub 6)[fail]"
        ${SETCOLOR_NORMAL}
        echo
fi
