...
- Ротация готовых копий бекапов (поиск и удаление старых дампов).

Всё, что вам нужно сделать - это скопировать скрипт на сервер, затем сделать скрипт исполняемым:
```markdown
